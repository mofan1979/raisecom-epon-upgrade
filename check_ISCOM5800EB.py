"""
    ...
    2. 目前只支持iscom5800EB OLT
    3. olt_list.csv必须和程序文件放在同一目录下，且不能改名。
    4. OLT设备ip列表放在/olt_list.txt供程序读取设备ip、telnet帐号、密码，每台OLT一行详见具体文件。
    5. 查询结果日志放在同个目录下，文件名为“result_日期_时间.csv”
    6. 程序为多进程并行升级，需根据PC性能配置决定并发进程数
"""
...
class Olt:
    ...
    def login(self):
        try:
            ...
            self.tn.write(self.telnetpw.encode())
    ...
    def check_type(self):
        ...
        if self.login_flag:
            ...
            logging.warning('%s 登陆失败，设备离线或型号不对或密码错误' % self.ip)
            self.logout()
            return '%s,登陆失败，设备离线或型号不对或密码错误\n' % self.ip


def multi_check():
    # 获取当前工作目录
    cwd = 'D:/github/raisecom-epon-upgrade'
